...
class Cate{
    ...
    async findAll_Paging(ctx,next){ 
        const total = await cateModel.find({}).count()
    ...
    async findAll_notPaging(ctx,next){
        const list = await cateModel.find({}).sort({addTime: -1})
        ctx.body = {
            data: {
                list
            },
    ...
    async edit(ctx,next){ 
        ...
        const result = await cateModel.updateOne({ _id }, { ...data, status })
    ...
    async remove(ctx,next){
        const _id = ctx.request.query.id
        const result = await cateModel.remove({ _id })
    ...
    async add(ctx,next){
        ...
        const status = review === '审核' ? 1 : 0  
        const navEntity = new cateModel({...data, status})
